...
* Description : This file contains create operation releated functions for department schema
**/
...
module.exports =  class createDepartmentService{
    ...
    * Description  : This function stores a record in department table
    **/
	async createOneDepartment(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return departmentMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Department.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Department.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_DEPARTMENT_CREATED_SUCCESSFULLY);
				}
				else {
