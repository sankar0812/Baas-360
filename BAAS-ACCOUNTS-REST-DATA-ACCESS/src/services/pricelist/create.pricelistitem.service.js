...
* Description : This file contains create operation releated functions for pricelistitem schema
**/
...
module.exports =  class createPriceListItemService{
    ...
    * Description  : This function stores a record in pricelistitem table
    **/
	async createOnePriceListItem(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return priceListMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.PriceListItem.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.PriceListItem.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_PRICE_LIST_ITEM_CREATED_SUCCESSFULLY);
				}
				else {
