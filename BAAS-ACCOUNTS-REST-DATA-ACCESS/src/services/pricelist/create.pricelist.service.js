...
* Description : This file contains create operation releated functions for pricelist schema
**/
...
module.exports =  class createPriceListService{
    ...
    * Description  : This function stores a record in pricelist table
    **/
	async createOnePriceList(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return priceListMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.PriceList.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.PriceList.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_PRICE_LIST_CREATED_SUCCESSFULLY);
				}
				else {
