...
* Description : This file contains create operation releated functions for currency schema
**/
...
module.exports =  class createCurrencyService{
    ...
    * Description  : This function stores a record in currency table
    **/
	async createOneCurrency(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return currencyMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Currency.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Currency.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_CURRENCY_CREATED_SUCCESSFULLY);
				}
				else {
