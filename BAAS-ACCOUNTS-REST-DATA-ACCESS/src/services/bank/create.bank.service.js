...
* Description : This file contains create operation releated functions for bank schema
**/
...
module.exports =  class createBankService{
    ...
    * Description  : This function stores a record in bank table
    **/
	async createOneBank(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return bankMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Bank.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Bank.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_BANK_CREATED_SUCCESSFULLY);
				}
				else {
