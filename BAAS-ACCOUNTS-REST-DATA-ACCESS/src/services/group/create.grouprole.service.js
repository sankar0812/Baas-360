...
* Description : This file contains create operation releated functions for grouprole schema
**/
...
module.exports =  class createGroupRoleService{
    ...
    * Description  : This function stores a record in grouprole table
    **/
	async createOneGroupRole(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return groupMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.GroupRole.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.GroupRole.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_GROUP_ROLE_CREATED_SUCCESSFULLY);
				}
				else {
