...
* Description : This file contains create operation releated functions for projecttype schema
**/
...
module.exports =  class createProjectTypeService{
    ...
    * Description  : This function stores a record in projecttype table
    **/
	async createOneProjectType(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return projectMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.ProjectType.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.ProjectType.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_PROJECT_TYPE_CREATED_SUCCESSFULLY);
				}
				else {
