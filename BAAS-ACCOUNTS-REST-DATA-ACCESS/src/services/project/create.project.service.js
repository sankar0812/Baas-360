...
* Description : This file contains create operation releated functions for project schema
**/
...
module.exports =  class createProjectService{
    ...
    * Description  : This function stores a record in project table
    **/
	async createOneProject(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return projectMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Project.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Project.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_PROJECT_CREATED_SUCCESSFULLY);
				}
				else {
