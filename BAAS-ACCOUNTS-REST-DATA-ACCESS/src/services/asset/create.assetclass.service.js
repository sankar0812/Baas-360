...
* Description : This file contains create operation releated functions for assetclass schema
**/
...
module.exports =  class createAssetClassService{
    ...
    * Description  : This function stores a record in assetclass table
    **/
	async createOneAssetClass(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return assetMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.AssetClass.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.AssetClass.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_ASSET_CLASS_CREATED_SUCCESSFULLY);
				}
				else {
