...
* Description : This file contains create operation releated functions for assetcategory schema
**/
...
module.exports =  class createAssetCategoryService{
    ...
    * Description  : This function stores a record in assetcategory table
    **/
	async createOneAssetCategory(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return assetMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.AssetCategory.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.AssetCategory.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_ASSET_CATEGORY_CREATED_SUCCESSFULLY);
				}
				else {
