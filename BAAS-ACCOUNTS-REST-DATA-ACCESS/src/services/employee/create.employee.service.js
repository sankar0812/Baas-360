...
* Description : This file contains create operation releated functions for employee schema
**/
...
module.exports =  class createEmployeeService{
    ...
    * Description  : This function stores a record in employee table
    **/
	async createOneEmployee(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return employeeMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Employee.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Employee.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_EMPLOYEE_CREATED_SUCCESSFULLY);
				}
				else {
