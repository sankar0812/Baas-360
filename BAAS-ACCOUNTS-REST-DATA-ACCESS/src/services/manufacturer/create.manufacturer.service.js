...
* Description : This file contains create operation releated functions for manufacturer schema
**/
...
module.exports =  class createManufacturerService{
    ...
    * Description  : This function stores a record in manufacturer table
    **/
	async createOneManufacturer(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return manufacturerMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Manufacturer.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Manufacturer.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_MANUFACTURER_CREATED_SUCCESSFULLY);
				}
				else {
