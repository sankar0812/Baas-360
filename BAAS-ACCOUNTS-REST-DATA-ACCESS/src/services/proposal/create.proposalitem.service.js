...
* Description : This file contains create operation releated functions for proposalitem schema
**/
...
module.exports =  class createProposalItemService{
    ...
    * Description  : This function stores a record in proposalitem table
    **/
	async createOneProposalItem(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return proposalMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.ProposalItem.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.ProposalItem.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_PROPOSAL_ITEM_CREATED_SUCCESSFULLY);
				}
				else {
