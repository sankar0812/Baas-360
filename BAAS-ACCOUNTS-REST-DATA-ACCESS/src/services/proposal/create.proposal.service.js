...
* Description : This file contains create operation releated functions for proposal schema
**/
...
module.exports =  class createProposalService{
    ...
    * Description  : This function stores a record in proposal table
    **/
	async createOneProposal(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return proposalMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Proposal.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Proposal.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_PROPOSAL_CREATED_SUCCESSFULLY);
				}
				else {
