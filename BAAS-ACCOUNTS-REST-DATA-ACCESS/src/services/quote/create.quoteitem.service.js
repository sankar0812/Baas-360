...
* Description : This file contains create operation releated functions for quoteitem schema
**/
...
module.exports =  class createQuoteItemService{
    ...
    * Description  : This function stores a record in quoteitem table
    **/
	async createOneQuoteItem(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return quoteMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.QuoteItem.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.QuoteItem.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_QUOTE_ITEM_CREATED_SUCCESSFULLY);
				}
				else {
