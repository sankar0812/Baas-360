...
* Description : This file contains create operation releated functions for vendor schema
**/
...
module.exports =  class createVendorService{
    ...
    * Description  : This function stores a record in vendor table
    **/
	async createOneVendor(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return vendorMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Vendor.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Vendor.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_VENDOR_CREATED_SUCCESSFULLY);
				}
				else {
