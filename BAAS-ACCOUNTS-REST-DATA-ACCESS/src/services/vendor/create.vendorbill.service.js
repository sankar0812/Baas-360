...
* Description : This file contains create operation releated functions for vendorbill schema
**/
...
module.exports =  class createVendorBillService{
    ...
    * Description  : This function stores a record in vendorbill table
    **/
	async createOneVendorBill(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return vendorMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.VendorBill.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.VendorBill.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_VENDOR_BILL_CREATED_SUCCESSFULLY);
				}
				else {
