...
* Description : This file contains create operation releated functions for vendorproduct schema
**/
...
module.exports =  class createVendorProductService{
    ...
    * Description  : This function stores a record in vendorproduct table
    **/
	async createOneVendorProduct(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return vendorMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.VendorProduct.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.VendorProduct.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_VENDOR_PRODUCT_CREATED_SUCCESSFULLY);
				}
				else {
