...
* Description : This file contains create operation releated functions for category schema
**/
...
module.exports =  class createCategoryService{
    ...
    * Description  : This function stores a record in category table
    **/
	async createOneCategory(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return categoryMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Category.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Category.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_CATEGORY_CREATED_SUCCESSFULLY);
				}
				else {
