...
* Description : This file contains create operation releated functions for tenant schema
**/
...
module.exports =  class createTenantService{
    ...
    * Description  : This function stores a record in tenant table
    **/
	async createOneTenant(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return tenantMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Tenant.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Tenant.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_TENANT_CREATED_SUCCESSFULLY);
				}
				else {
