...
* Description : This file contains create operation releated functions for auth schema
**/
...
module.exports =  class createAuthService{
    ...
    * Description  : This function stores a record in auth table
    **/
	async createOneAuth(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return authMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Auth.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Auth.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_AUTH_CREATED_SUCCESSFULLY);
				}
				else {
