...
* Description : This file contains create operation releated functions for productpackage schema
**/
...
module.exports =  class createProductPackageService{
    ...
    * Description  : This function stores a record in productpackage table
    **/
	async createOneProductPackage(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return productMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.ProductPackage.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.ProductPackage.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_PRODUCT_PACKAGE_CREATED_SUCCESSFULLY);
				}
				else {
