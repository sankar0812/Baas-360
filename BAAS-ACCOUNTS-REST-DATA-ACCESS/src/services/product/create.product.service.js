...
* Description : This file contains create operation releated functions for product schema
**/
...
module.exports =  class createProductService{
    ...
    * Description  : This function stores a record in product table
    **/
	async createOneProduct(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return productMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Product.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Product.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_PRODUCT_CREATED_SUCCESSFULLY);
				}
				else {
