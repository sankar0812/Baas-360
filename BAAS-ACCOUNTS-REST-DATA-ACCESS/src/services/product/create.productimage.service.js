...
* Description : This file contains create operation releated functions for productimage schema
**/
...
module.exports =  class createProductImageService{
    ...
    * Description  : This function stores a record in productimage table
    **/
	async createOneProductImage(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return productMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.ProductImage.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.ProductImage.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_PRODUCT_IMAGE_CREATED_SUCCESSFULLY);
				}
				else {
