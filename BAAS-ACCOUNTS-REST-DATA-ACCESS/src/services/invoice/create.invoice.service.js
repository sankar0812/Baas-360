...
* Description : This file contains create operation releated functions for invoice schema
**/
...
module.exports =  class createInvoiceService{
    ...
    * Description  : This function stores a record in invoice table
    **/
	async createOneInvoice(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return invoiceMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Invoice.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Invoice.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_INVOICE_CREATED_SUCCESSFULLY);
				}
				else {
