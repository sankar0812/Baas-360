...
* Description : This file contains create operation releated functions for brand schema
**/
...
module.exports =  class createBrandService{
    ...
    * Description  : This function stores a record in brand table
    **/
	async createOneBrand(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return brandMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Brand.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Brand.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_BRAND_CREATED_SUCCESSFULLY);
				}
				else {
