...
* Description : This file contains create operation releated functions for role schema
**/
...
module.exports =  class createRoleService{
    ...
    * Description  : This function stores a record in role table
    **/
	async createOneRole(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return roleMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Role.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Role.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_ROLE_CREATED_SUCCESSFULLY);
				}
				else {
