...
* Description : This file contains create operation releated functions for location schema
**/
...
module.exports =  class createLocationService{
    ...
    * Description  : This function stores a record in location table
    **/
	async createOneLocation(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return locationMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Location.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Location.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_LOCATION_CREATED_SUCCESSFULLY);
				}
				else {
