...
* Description : This file contains create operation releated functions for contactperson schema
**/
...
module.exports =  class createContactPersonService{
    ...
    * Description  : This function stores a record in contactperson table
    **/
	async createOneContactPerson(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return contactMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.ContactPerson.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.ContactPerson.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_CONTACT_PERSON_CREATED_SUCCESSFULLY);
				}
				else {
