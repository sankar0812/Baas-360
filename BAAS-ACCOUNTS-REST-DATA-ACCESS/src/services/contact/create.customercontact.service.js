...
* Description : This file contains create operation releated functions for customercontact schema
**/
...
module.exports =  class createCustomerContactService{
    ...
    * Description  : This function stores a record in customercontact table
    **/
	async createOneCustomerContact(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return contactMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.CustomerContact.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.CustomerContact.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_CUSTOMER_CONTACT_CREATED_SUCCESSFULLY);
				}
				else {
