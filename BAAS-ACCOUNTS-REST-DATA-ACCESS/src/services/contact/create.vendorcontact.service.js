...
* Description : This file contains create operation releated functions for vendorcontact schema
**/
...
module.exports =  class createVendorContactService{
    ...
    * Description  : This function stores a record in vendorcontact table
    **/
	async createOneVendorContact(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return contactMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.VendorContact.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.VendorContact.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_VENDOR_CONTACT_CREATED_SUCCESSFULLY);
				}
				else {
