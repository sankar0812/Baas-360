...
* Description : This file contains create operation releated functions for contact schema
**/
...
module.exports =  class createContactService{
    ...
    * Description  : This function stores a record in contact table
    **/
	async createOneContact(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return contactMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Contact.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Contact.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_CONTACT_CREATED_SUCCESSFULLY);
				}
				else {
