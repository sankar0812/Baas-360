...
* Description : This file contains create operation releated functions for customer schema
**/
...
module.exports =  class createCustomerService{
    ...
    * Description  : This function stores a record in customer table
    **/
	async createOneCustomer(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return customerMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Customer.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Customer.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_CUSTOMER_CREATED_SUCCESSFULLY);
				}
				else {
