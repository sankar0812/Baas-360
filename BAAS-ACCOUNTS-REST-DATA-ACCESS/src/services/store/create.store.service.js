...
* Description : This file contains create operation releated functions for store schema
**/
...
module.exports =  class createStoreService{
    ...
    * Description  : This function stores a record in store table
    **/
	async createOneStore(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return storeMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Store.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Store.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_STORE_CREATED_SUCCESSFULLY);
				}
				else {
