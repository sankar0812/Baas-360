...
* Description : This file contains create operation releated functions for storetype schema
**/
...
module.exports =  class createStoreTypeService{
    ...
    * Description  : This function stores a record in storetype table
    **/
	async createOneStoreType(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return storeMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.StoreType.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.StoreType.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_STORE_TYPE_CREATED_SUCCESSFULLY);
				}
				else {
