...
* Description : This file contains create operation releated functions for merchant schema
**/
...
module.exports =  class createMerchantService{
    ...
    * Description  : This function stores a record in merchant table
    **/
	async createOneMerchant(databaseConnector, apiID, config, input) {
		try {
			if (config.IsAvailable == false) { return Commons.generateServiceOutputForAvalibalityNotEnabled() }
			if (config.IsMockEnabled == true) { return merchantMock[apiID] }
			else {
				let duplicateRecord = await databaseConnector.Merchant.findMany({
				...
				if (duplicateRecord.length === 0 || Object.keys(filters).length === 0) {
					...
					let output = await databaseConnector.Merchant.create({
						data: input.data,
					}).catch((error) => {
						if (error instanceof Prisma.PrismaClientValidationError) {
							throw Commons.generateServiceOutput(null, 422, JSON.stringify(error.message))
					...
					return Commons.generateServiceOutput(output, 200, Messages.MESSAGE_MERCHANT_CREATED_SUCCESSFULLY);
				}
				else {
