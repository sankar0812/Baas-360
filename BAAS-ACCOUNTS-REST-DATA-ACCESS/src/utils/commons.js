...
module.exports = class commons {
	...
	**/
	validateRequestHeaders(merchantKey, tenantKey, userKey) {
		if (typeof merchantKey === "undefined" || typeof tenantKey === "undefined" || typeof userKey === "undefined" || !merchantKey || !tenantKey || !userKey) {
	...
	**/
	async executeController(request, response, apiID, serviceFunction) {
		...
		try {
			...
			const merchatKey = request.headers.merchantkey; const tenantKey = request.headers.tenantkey; const userKey = request.headers.userkey;
	...
	**/
	getDatabaseConnection(merchantKey, tenantKey) {
		if (merchantKey === undefined || tenantKey === undefined) {
			throw Messages.MESSAGE_HEADER_REQUIRED;
		}
		const databaseConnection = Config.ACCOUNTS_REST_DATA_ACCESS_DATABASE_CONNECTIONS
